...
void varcpy(VarDesc* source, VarDesc** destination, VarDesc** new_tail) {
    VarDesc* current = source;
    VarDesc* newHead = NULL;
    VarDesc* newTail = NULL;

    while (current != NULL) {
        VarDesc* newNode = (VarDesc*)malloc(sizeof(VarDesc));
        memcpy(newNode, current, sizeof(VarDesc));
        newNode->next = NULL;
...
void save_state(int labelno) {
    State *new_state = (State*)malloc(sizeof(State));
    new_state->regs = (RegDesc*)malloc(sizeof(RegDesc) * NUM_REGS);
    new_state->vars = NULL;
    memcpy(new_state->regs, regs, sizeof(RegDesc) * NUM_REGS);
    varcpy(vars, &new_state->vars, &new_state->vars_tail);
    new_state->offset = offset;
    new_state->cur_reg = cur_reg;
    new_state->labelno = labelno;
    if (state == NULL) {
        state = new_state;
        state_tail = new_state;
    } else {
        state_tail->next = new_state;
        state_tail = new_state;
    }
}

void load_state(int labelno) {
    State *s = state;
    while (s != NULL && s->labelno != labelno) {
        s = s->next;
    }
    assert(s != NULL);
    memcpy(state->regs, regs, sizeof(RegDesc) * NUM_REGS);
    varcpy(vars, &state->vars, &state->vars_tail);
    state->offset = offset;
...
tac *emit_label(tac *label){
    ...
    load_state(_tac_quadruple(label).labelno->int_val);
...
tac *emit_goto(tac *goto_){
    save_state(_tac_quadruple(goto_).labelno->int_val);
...
tac *emit_iflt(tac *iflt){
    ...
    save_state(_tac_quadruple(iflt).labelno->int_val);
    Register x, y;
...
tac *emit_ifle(tac *ifle){
    /* COMPLETE emit function */
    save_state(_tac_quadruple(ifle).labelno->int_val);
    Register x, y;
...
tac *emit_ifgt(tac *ifgt){
    /* COMPLETE emit function */
    save_state(_tac_quadruple(ifgt).labelno->int_val);
    Register x, y;
...
tac *emit_ifge(tac *ifge){
    /* COMPLETE emit function */
    save_state(_tac_quadruple(ifge).labelno->int_val);
    Register x, y;
...
tac *emit_ifne(tac *ifne){
    /* COMPLETE emit function */
    save_state(_tac_quadruple(ifne).labelno->int_val);
...
tac *emit_ifeq(tac *ifeq){
    /* COMPLETE emit function */
    save_state(_tac_quadruple(ifeq).labelno->int_val);
