%{
    ...
    char* translate_StmtList(TreeNode* StmtList);

    void pushCode(char* code);
    char* popCode();
    void pushRegister(char* place);
    char* popRegister();
%}
%union {
    struct {
...
void pushRegister(char* place){
    registerStack[registerDepth++] = place;
}

char* popRegister(){
    return registerStack[--registerDepth];
}
...
void translate_Exp_INT(TreeNode* INT, char* place){
    ...
    // TODO char* code = "";
    sprintf(code, "%s := #%s", place, INT -> value);
    pushCode(code);
    pushRegister(place);
}
...
void translate_Exp_ID(TreeNode* ID, char* place){
    char* code1 = get(ID->value)->registerName;
    char* code = (char*)calloc(30);
    // TODO char* code = "";
    sprintf(code, "%s := %s", place, code1);
    pushCode(code);
    pushRegister(place);
}
...
void translate_Exp_ASSIGN(TreeNode* Exp, char* place){
    // char* var = get(Exp -> children[0] -> children[0] -> value) -> registerName;
    char* code1 = popCode();
    char* reg1 = popRegister();
    char* code2 = popCode();
    char* reg2 = popRegister();

    // char* code1 = translate_Exp(Exp -> children[2], var2);
    char* code3 = (char*)calloc(30, 1); sprintf(code2, "%s := %s\n", reg2, reg1);
    char* code4 = (char*)calloc(30, 1); sprintf(code3, "%s := %s\n", place, reg2);
...
void translate_Exp_MINUS(TreeNode* Exp, char* place){
    ...
    char* code1 = popCode();
    char* reg1 = popRegister();
    char* code2 = (char*)calloc(30,1); sprintf(code2, "%s := #0 - %s", place, reg1);
...
char* translate_Stmt(TreeNode* Stmt){
    /* if(Stmt->numChildren == 2 && strcmp(Stmt->children[1]->type, "SEMI") == 0){
        char* place = new_place('t');
        return translate_Exp(Stmt->children[0], place);
    } */
    printf("stmt %d\n", Stmt->line);
    if(Stmt->numChildren == 3 && strcmp(Stmt->children[0]->type, "RETURN") == 0){
        ...
        return translate_StmtList(Stmt->children[0]->children[2]);
    }
}

char* translate_StmtList(TreeNode* StmtList){
    char* code1 = translate_Stmt(StmtList->children[0]);
    printf("here\n");
    char* code2 = translate_StmtList(StmtList->children[2]);
    char* code = (char*)calloc(strlen(code1) + strlen(code2), 1);
    sprintf(code, "%s%s", code1, code2);
    free(code1);
    free(code2);
    return code;
